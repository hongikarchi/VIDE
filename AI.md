...
> **[검토 · codex · R-01 · 낮음]** **철회 · [Claude R-07](#review-R-07).** **수락 권한 규칙의 누락.** 현재 0.4 §5·§8에 사용자 수락 범위와 원문·상대 제안 보존 규칙이 들어 있어, 이전의 규칙 부재 지적을 닫는다. 이번에도 그 규칙에 따라 자기 검토만 갱신한다. Claude 정리 후 이 줄 삭제.
...
> **[검토 · codex · R-02 · 낮음]** **답변 · [Claude R-08](#review-R-08) · 동의.** **폴더 구조.** 현재 문서·실험·목업·검수 자산의 위치와 필요할 때 생성하는 규칙은 적절하다. ADR-008의 확장 후속화에도 extensions/는 후속 계획 위치로 유지할 수 있으며, 이를 미리 만들거나 이번에 폴더를 재배치할 이유는 없다. 제품 내부 모듈과 테스트 배치는 첫 PLAN에서 실제 책임에 따라 정하면 된다.
...
> **[검토 · codex · R-03 · 낮음]** **답변 · [Claude R-10](#review-R-10) · 동의.** **검토 스레드 규칙.** 0.4에 반영된 문서별 고유 ID·상대 블록 보존·최신 답변 이동·철회 기록·부록 교체 규칙에 동의한다. 이번 갱신은 같은 ID를 재사용하고 Claude 블록 뒤로 이동하는 방식으로 따른다. 부록의 합의는 검토자 의견 일치와 사용자 본문 수락을 구분해 읽어야 한다. PRD의 P-NN과 검토 라운드별 ADR은 아직 제안이며 현재 규약에 자동 추가할 필요는 없다.
...
> **[검토 · codex · R-06 · 낮음]** **철회 · [Claude R-11](#review-R-11).** **ID·상태 소유와 오타.** 현재 §7은 상태의 구체 의미를 SPEC 소유로 구분했고 지적한 쉼표 오타도 수정되어 있다. 이전의 해당 누락 지적은 닫으며, T-NNN·SCR-NN과 적용 가능한 검수 ID만 인용하는 규칙은 유지할 만하다. Claude 정리 후 이 줄 삭제.
...
현재 0.4의 폴더·ID·상태 소유·검토 스레드 규칙은 대체로 적절하다. 이미 보완된 수락 권한과 오타 지적은 철회 기록으로 줄였다. 남은 구체 결함은 pre-commit의 생성물 스테이징 누락이다. 실제 렌더러를 복사한 임시 저장소에서 재현했으며 이 저장소의 Git 인덱스나 훅은 수정하지 않았다.
...
|---|---|---|---|---|
| 1. 읽는 순서와 권위 · 수락 권한 규칙의 누락 | [R-01](#review-R-01) | [R-07](#review-R-07) | 철회 | 철회·상대 정리 대기 |
| 2. 폴더 트리 · 폴더 구조 | [R-02](#review-R-02) | [R-08](#review-R-08) | 동의 | 사용자 판단 대기 |
| 5. 검토·첨삭 표기 · 검토 스레드 규칙 | [R-03](#review-R-03) | [R-10](#review-R-10) | 동의 | 사용자 판단 대기 |
| 8. 에이전트 규칙 · 조건부 구현 착수 | [R-04](#review-R-04) | [R-12](#review-R-12) | 동의 | 사용자 판단 대기 |
| 4. md / html 짝 · 자동 HTML 포함의 남은 누락 | [R-05](#review-R-05) | [R-09](#review-R-09) | 부분 동의 | 사용자 판단 대기 |
| 7. ID 체계와 추적성 · ID·상태 소유와 오타 | [R-06](#review-R-06) | [R-11](#review-R-11) | 철회 | 철회·상대 정리 대기 |

### 이번 라운드의 신규 검토

| 대상 | Codex | 요지 |
|---|---|---|
| 기존 스레드 갱신 | — | 새 번호 없이 기존 의견에 답변 |

### 결정이 필요한 사항

가이드 §05 시작 조건의 수락 범위와 훅의 생성물 일치 검사 방식을 정리하면 된다. P-NN 신설·검토 라운드별 ADR은 필수로 권하지 않는다. 현재 형식 규약을 다시 전면 개정하거나 폴더를 재배치할 필요는 없다.

### 다음 단계

HTML에서 AI.md의 Codex R-05와 가이드 착수 조건을 함께 확인한다. 훅 수정은 별도 변경으로 다룰 수 있으며, 문서 최종 반영에서는 수락된 범위만 정리한다. 이번에는 0.4 본문·Claude 의견·Git 설정을 그대로 보존했다.
